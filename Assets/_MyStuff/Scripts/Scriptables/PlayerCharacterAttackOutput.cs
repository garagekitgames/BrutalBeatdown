            ...
            if (attack)
            {
                ...
                if (attackTimer < currentAttack.attackSpeed * currentAttack.attackForceApplyPercent)
                {
                    if (attackTimer < currentAttack.attackSpeed * (currentAttack.attackForceApplyPercent - currentAttack.attackForceApplyPercent * currentAttack.accuracyReduceFactor))
                    {
                        ...
                        if(character.objectOfInterest)
                        {
                            Vector3 a = (character.objectOfInterest.cachedCollider.ClosestPoint(character.bpHolder.bodyParts[currentAttack.bodyPartToHitWith].BodyPartRb.transform.position) - character.bpHolder.bodyParts[currentAttack.bodyPartToHitWith].BodyPartRb.transform.position).normalized;
                            attackTarget = a;
                        }
                        else
                        {
                            attackTarget = (character.target - character.bpHolder.bodyParts[currentAttack.bodyPartToHitWith].BodyPartTransform.position).normalized;


                        }
                        // Vector3 a = (target.cachedCollider.ClosestPoint(handsBody[0].transform.position) - handsBody[0].transform.position).normalized;
                    ...
                    if (character.target != null)
                    {
                        ...
                        foreach(AttackForceRatio forceRatio in currentAttack.attackForceRatio)
                        {
                            ...
                            //Debug.Log("forceRatio.forceRatio : " + forceRatio.forceRatio);
                        }

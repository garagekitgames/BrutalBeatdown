...
public class Punching : MonoBehaviour {
    ...
    public string attackButton = "SimplePrimaryAttack";
    ...
    private void FixedUpdate()
    {
        ...
        if (leftWindUp)
        {
           // character.windUp = true;
            if (attackSide == 0)
            {
                JointDrive x = armsJoint[attackSide].slerpDrive;
                x.positionDamper = 100f;
                x.positionSpring = 1000f;
                x.maximumForce = 1000f;
                armsJoint[attackSide].slerpDrive = x;

                Vector3 jointValue = new Vector3(20f, 0f, 0f);

                armsJoint[attackSide].targetAngularVelocity = jointValue;

                JointDrive x1 = forearmsJoint[attackSide].slerpDrive;
                x1.positionDamper = 100f;
                x1.positionSpring = 1000f;
                x1.maximumForce = 1000f;
                forearmsJoint[attackSide].slerpDrive = x1;

                Vector3 jointValue1 = new Vector3(-20f, 0f, 0f);

                forearmsJoint[attackSide].targetAngularVelocity = jointValue1;
                if (windupDrag)
        ...
        if (leftpunch)
        {
            ...
            if (leftpunchTimer < punchSpeed * punchForceApplyPercent)
            {
                ...
                {
                    leftPunchTarget = (target - handsBody[attackSide].transform.position).normalized;
                ...
                if (target != null)
                {
                    ...
                    */
                    ...
                    armsBody[attackSide].AddForce(-(leftPunchTarget * punchPower * 0.34f) * Time.deltaTime, ForceMode.VelocityChange);
                    handsBody[attackSide].AddForce(leftPunchTarget * punchPower * 1f * Time.deltaTime, ForceMode.VelocityChange);
        ...
        //character.attackTarget = leftPunchTarget;
        ////character.windTimer = leftWindTimer;
        //character.attackTimer = leftpunchTimer;
        //character.windUp = leftWindUp;
        //character.attack = leftpunch;
    }
}

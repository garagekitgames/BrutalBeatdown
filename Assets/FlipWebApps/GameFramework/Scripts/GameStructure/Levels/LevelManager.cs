...
namespace GameFramework.GameStructure.Levels
{
    ...
    public class LevelManager : Singleton<LevelManager>
    {
        ...
        public float threeStarTimeTarget = 10;

        public float twoStarTimeTarget = 20;

        public bool useTimeTargetForStars = false;

        public bool useCollectibleStars = true;
        public float levelTimeAllowed = 20f;
        ...
        public void GetStarsDetails()
        {
            //if (GameManager.Instance.Player.Health >= 0 && GameManager.Instance.Player.Health < 0.3f)
            //{
            //    Level.StarWon(1, false);
            //    Level.StarWon(2, false);
            //    Level.StarWon(3, false);
            //}

            if(useTimeTargetForStars)
            {
                if(levelExitReached)
                {
                    Level.StarWon(1, true);

                    if (SecondsRunning <= threeStarTimeTarget)
                    {
                        Level.StarWon(2, true);
                        Level.StarWon(3, true);

                    }
                    if(SecondsRunning <= twoStarTimeTarget)
                    {
                        Level.StarWon(2, true);
                    }
                ...
                //if (SecondsRunning > twoStarTimeTarget && levelExitReached)
                //    Level.StarWon(1, true);
                //if (SecondsRunning > threeStarTimeTarget && levelExitReached)
                //    Level.StarWon(2, true);
                //if (SecondsRunning <= threeStarTimeTarget && levelExitReached)
                //    Level.StarWon(3, true);
            }
            else if(useCollectibleStars)
            {
                if (collectedStars >= 1)
                    Level.StarWon(1, true);
                if (collectedStars >= 2)
            ...
            else
            {
                if (GameManager.Instance.Player.Health > 0)
                    Level.StarWon(1, true);
                if (GameManager.Instance.Player.Health >= 0.6f)
                    Level.StarWon(2, true);
                if (GameManager.Instance.Player.Health >= 0.8f)
                    Level.StarWon(3, true);
            }
            ...
            //}
            //else if(GameManager.Instance.Player.Health >= 0.6f && GameManager.Instance.Player.Health < 0.8f)
            //{
            //    //Level.StarsWon = 2;
            //    Level.StarWon(1, true);
            //    Level.StarWon(2, true);
            //    Level.StarWon(3, false);
            //}
            //else if (GameManager.Instance.Player.Health >= 0.3f && GameManager.Instance.Player.Health < 0.6f)
            //{
            //    //Level.StarsWon = 1;
            //    Level.StarWon(1, true);
            //    Level.StarWon(2, false);
            //    Level.StarWon(3, false);
            //}
            ////else if (GameManager.Instance.Player.Health >= 0 && GameManager.Instance.Player.Health < 0.3f)
            ////{
            ////    Level.StarWon(1, false);
            ////    Level.StarWon(2, false);
            ////    Level.StarWon(3, false);
        ...
        public void SetLevelExitReached(bool value)
        {
            levelExitReached = value;
        }

        public void SetPlayerCaught(bool value)
        {
            playerWasCaught = value;
        }
        ...
        void Update()
        {
            if (IsLevelRunning)
            {
                    ...
                    (playerWasCaught == true))
                {
                    ...
                    AudioManager.instance.FadeInCaller("BGM1", 0.1f, 0.5f);
                ...
                if ((GameWonWhenAllStarsGot && Level.StarTotalCount == Level.StarsWonCount && StartStarsWon != Level.StarsWon) ||
                   // GameWonWhenTargetScoreReached && false || //Level.Score >= LevelGenerator.Instance.enemyCount ||//Level.ScoreTarget ||

    ...
    public class GameManager : SingletonPersistant<GameManager>
    {
        ...
        public IEnumerator SyncLevelsFromServer()
        {
            ...
            if (cloud.isError)
            {
                ...
                int myLevelCount =25;
                if (ES3.KeyExists("levelCount"))
                {
                    myLevelCount = ES3.Load<int>("levelCount");
                }
                else
                {
                    myLevelCount = 25;
                }
                int levelCount = myLevelCount;
                LevelSetupMode = GameManager.GameItemSetupMode.Automatic;
                NumberOfAutoCreatedLevels = levelCount - 1;
            ...
            {
                int levelCount = cloud.filenames.Length;

                ES3.Save<int>("levelCount", levelCount);
                LevelSetupMode = GameManager.GameItemSetupMode.Automatic;
                NumberOfAutoCreatedLevels = levelCount - 1;

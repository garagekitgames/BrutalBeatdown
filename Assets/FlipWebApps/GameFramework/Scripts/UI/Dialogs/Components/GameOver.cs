...
namespace GameFramework.UI.Dialogs.Components
{
    ...
    // record some analytics on the level played
        ...
        public void Continue()
        {
            ...
            //AdManager.Instance.ShowIterstitialAd();
            ...
            if (GameManager.Instance.Levels.GetNextItem() != null)
            {
                if (GameManager.Instance.Levels.GetNextItem().IsUnlocked)
                {
                    ...
                    ContinueScene = "Game_"+ GameManager.Instance.Levels.GetNextItem().Number;
                    GameManager.Instance.Levels.SetSelected(GameManager.Instance.Levels.GetNextItem());
                }
                else
                {
                    ContinueScene = "Game_" + GameManager.Instance.Levels.Selected.Number;
                }
            }
            else if(GameHelper.IsCurrentLevelLastInGame())
            {
                //nextLevelUnlocked = true;
                ContinueScene = "Game_"+"1";
                GameManager.Instance.Levels.SetSelected(1);
            }
            else
            {
                ContinueScene = "Game_"+ GameManager.Instance.Levels.Selected.Number;

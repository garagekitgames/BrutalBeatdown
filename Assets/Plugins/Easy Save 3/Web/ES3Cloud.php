...
else if(isset($_POST["postFile"]))
{
	...
	// $strArray = explode("_",$file["name"]);
	// $lastElement = (int)end($strArray);
	// echo $lastElement;
	// $stmt->bindParam(":levelKey", $lastElement);
	$stmt->bindParam(":data", $fp, PDO::PARAM_LOB);
	$postUser = GetPOSTUser();
	$stmt->bindParam(":user", $postUser);
	$postTimestamp = GetPOSTTimestamp();
...
{
	$stmt = $db->prepare("SELECT $filenameField FROM $tableName WHERE $userField = :user ORDER BY LENGTH($filenameField), $filenameField");
...
// ----- GET and Display FILENAMES -----
else if(isset($_POST["getAndShowFilenames"]))
{
	$stmt = $db->prepare("SELECT $filenameField FROM $tableName WHERE $userField = :user ORDER BY LENGTH($filenameField), $filenameField");
	$stmt->bindParam(":user", GetPOSTUser());
	$stmt->execute();
